...
PARTICLE_TRAIL_COLOR = (128, 128, 128)
PARTICLE_TRAJECTORY_COLOR = (48, 48, 48)
...
class Display:
    ...
    def set_ssaa_enabled(self, enabled):
        ...
        surface_size = (screen_size[0] * self.ssaa_factor,
                        screen_size[1] * self.ssaa_factor)
    ...
    def render(self, simulation, camera):
        ...
        for particle_id in particles.keys():
            self.draw_particle_trail(
                particle_id, particles, particles_history, camera)
    ...
    def draw_particle_trail(self, particle_id, current_particles, particles_history, camera):
        future_screen_position = None
        color = PARTICLE_TRAIL_COLOR

        for snapshot in particles_history:
            if particle_id not in snapshot:
                break  # the end

            if snapshot == current_particles:
                color = PARTICLE_TRAJECTORY_COLOR
            ...
            if future_screen_position:
                self.draw_line(particle_screen_position,
                               future_screen_position, PARTICLE_TRAIL_WIDTH, color)
